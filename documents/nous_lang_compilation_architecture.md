...
2. `nous_parser` builds an AST for functions, typed parameters, `let`, assignment, returns, break/continue, if/elif/else, while/loop blocks, calls, literals, variables, arithmetic, and comparisons.
3. `nous_semantics` validates static types, local bindings, assignments, function calls, return behavior, bool conditions, loop-control placement, and the interim pointer-style memory builtins.
4. `nous_runtime` executes the validated AST directly, including `main`, calls, scoped locals, assignment, branch result values, while/loop control flow, arithmetic/comparisons, and heap-slot memory operations.
