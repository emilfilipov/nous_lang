...
| `L0454` | semantic | Wrong generic-type argument arity. | A generic user struct is spelled with the wrong number of type arguments — too many, too few, or none — for its declared type-parameter list (for example `Box<i64, bool>` or a bare `Box` for `struct Box<T>`). | Spell the generic type with exactly as many type arguments as it declares type parameters (`Box<i64>` for `struct Box<T>`). |
| `L0455` | semantic | Uninferable generic-struct type parameter. | Constructing a generic user struct leaves a type parameter unpinned: the arguments do not determine it (for example a parameter that appears in no field, or an empty/unpinnable field value) and no `Name<...>` annotation supplies it. | Add a type annotation such as `Box<i64>` on the binding, parameter, or return so the type argument is fixed. |
