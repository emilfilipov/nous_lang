...
- **`for x in f.field`** and **binding a whole field array to a local** (`let c =
  f.data`) — both need a field-place aggregate copy / a locals-aware length inference
  that is a separable follow-up; they demote the enclosing function cleanly.
...
`native_object_place.rs` (`resolve_path_array_len`), `native_object_expr.rs` (the
...
and by-value-parameter soundness core, narrow + nested layouts, and the two
