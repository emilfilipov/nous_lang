...
- **Now delivered:** the `min(a, b)`/`max(a, b)` builtins on `i64` — a branchless signed `cmp rcx, rax` + `cmovl`/`cmovg rax, rcx` (`48 0F 4C C1` / `48 0F 4F C1`), bit-exact with the interpreters' `i64::min`/`i64::max` (signed, total, no overflow). They compose across function boundaries (a `clampi lo hi x -> max(lo, min(hi, x))` helper compiles), verified by `native_min_max_execution_parity_when_linkable` (including negative operands) and `compiles_min_max_i64_as_cmp_cmov`. The `f64` case of `min`/`max` is **deferred** — the SSE `minsd`/`maxsd` NaN/`±0.0` tie-breaking rules diverge from `f64::min`/`f64::max`, so an f64 `min`/`max` skips gracefully to the interpreters (asserted by `min_max_f64_defers_gracefully`); a fixed-width kind cannot occur (the type checker admits only `i64`/`f64`). Still deferred: the remaining transcendental/rounding math builtins (`sin`/`cos`/`floor`/`ceil`/`round`/…) — `floor`/`ceil`/`round` need SSE4.1 `roundsd` behind a CPUID guard, the transcendentals need a libm call or a polynomial kernel — and `parse_f64`; a function using one still skips gracefully to the interpreters.
