...
- Optimizer support is intentionally conservative: constant folding, block-local copy propagation for simple aliases, and block-local dead-code elimination are implemented; CSE and loop-invariant motion are deferred to the next optimizer phase.

## Next Phase

The next phase should keep the working AST runtime and installable package intact while hardening the typed IR and bytecode backend with common subexpression elimination, loop-invariant motion, broader backend snapshots, and a lower-level bytecode instruction format.
