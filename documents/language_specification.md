...
- IR and bytecode execution can opt into the initial deterministic optimizer with `--optimize constant-fold`, `--optimize dead-code`, or `--optimize alpha`; the alpha pipeline currently includes constant folding, conservative CSE, conservative copy propagation, and dead-code elimination. The default is `--optimize none`.
- `nlang compile [--optimize none|constant-fold|dead-code|alpha] [-o output.nbc] <file.nl>` emits a versioned `.nbc` instruction-bytecode artifact with metadata and a function table, `nlang inspect <file.nbc>` summarizes that artifact, and `nlang run <file.nbc>` executes it after compatibility checks.
