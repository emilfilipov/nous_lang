...
- **Functions.** `fn name param Type -> ReturnType` (the `-> ReturnType` clause is **optional** — the return type is inferred from the body when omitted, except for a recursive function, which must state it; consecutive same-type parameters may be **comma-grouped**, so `x, y, z f64` is exactly `x f64 y f64 z f64`), last-expression return with explicit early `return`, `-> void` procedures, **first-class function values** (`fn(T) -> R` types; passing/returning functions), and **generic functions** (`fn name<T> ...` and bounded `<T: Trait>`) with call-site inference. `async fn`/`await`/`Future<T>` are supported on the interpreter backends.
...
- **Tooling.** A CLI (`check`/`compile`/`build`/`run`/`test`/`wasm`/`native`/`fmt`/`inspect`/`lsp`/`docs`/`examples`), a Language Server (`lullaby lsp`), and a hosted online documentation website (`lullaby docs` prints its URL).
