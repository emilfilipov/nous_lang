...
Once Alpha 1 is accepted, the next phase should harden the typed semantic IR and initial bytecode backend with remaining optimizer passes such as loop-invariant motion, backend snapshot tests, and a fuller instruction-bytecode VM before native code generation. This keeps the project conservative: preserve the working AST runtime while proving that a lower-level contract can support later optimization and native backends.
