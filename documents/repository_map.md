...
- `cargo run -p nous_cli -- run --backend bytecode --optimize alpha tests/fixtures/valid/run_arithmetic.nl`: run a valid fixture through typed IR lowering, the current alpha optimizer pipeline of constant folding, CSE, copy propagation, and dead-code elimination, instruction-bytecode lowering, and the bytecode VM entry point.
