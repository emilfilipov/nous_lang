...
- User-defined generic *types* (`struct Box<T>`, `enum Tree<T>`) and **explicit type arguments** at a call site (`identity<i64>(3)`). User-defined generic *functions* with call-site type inference are implemented (see [generics_design.md](generics_design.md)), and **trait bounds** (`<T: Ord>`) are implemented (see [traits_design.md](traits_design.md)) — a bounded `T` may call its bound traits' methods. Argument-position inference for `option`/`result` construction also remains planned.
