...
The repository now contains the following implemented slice:
...
- `lullaby_ir`: typed semantic IR schema, lowering from `CheckedProgram`, memory-operation analysis for allocation/load/store/deallocation and bounds-checked indexing with stable sequence metadata, deterministic optimization pass configuration with initial constant folding, block-local common subexpression elimination, conservative loop-invariant motion, copy propagation, and dead-code elimination, executable IR interpreter, explicit instruction-bytecode lowering, versioned `.lbc` artifact encoding/decoding with metadata/function-table/memory-operation compatibility checks, and bytecode VM entry point for the current implemented subset.
- `lullaby_cli`: exposes the `lullaby` binary with `check <file.lby>`, `compile [--optimize none|constant-fold|dead-code|full] [-o output.lbc] <file.lby>`, `build [--optimize none|constant-fold|dead-code|full] [-o output.lbc] <file.lby>`, `run [--backend ast|ir|bytecode] [--optimize none|constant-fold|dead-code|full] <file.lby|file.lbc>`, and `docs`.
...
- `offline_docs`: self-contained offline browser documentation entry point, generated documentation build path, and verifier for required documentation sections, local-only links/assets, fixture-backed executable examples, generated parity coverage, and release package coverage.
- `documents/language_surface.md`: earliest installable language, CLI, diagnostics, artifact, and packaging surface; broader design docs are planned unless they match this file.
- `documents/release_notes.md`: release notes with package contents, supported surface, commands, artifact contract, verification evidence, limitations, and next-phase guidance.
...
- `documents/acceptance_criteria.md`: release checklist covering required feature surface, docs, verification gates, evidence, non-goals, and recommended next phase.
- `documents/roadmap.md`: repo-owned execution guidance for memory-aware IR operations, generated offline docs, cross-platform build orchestration, installer packaging, and native code generation sequencing.
...
| :--- | :--- | :--- | :--- | :--- |
| **1.1** | **Lexer & Parser Implementation:** Implement the core components to read raw lullaby source code and convert it into an Abstract Syntax Tree (AST). | `lullaby_syntax_design.md` | High | Core functions/expressions/arrays/conditionals/loops/range-for done; formal grammar draft in `formal_grammar.md`; planned syntax keywords now reject with `L0211`; ongoing for full language |
| **1.2** | **Type System Integration:** Implement the type checker based on `lullaby_type_system.md`. Ensure all AST nodes are correctly typed and checked before execution. | `lullaby_type_system.md` | High | Scalar/array/call/assignment/control-flow/logical checks, executable zero-argument `main` validation for compile/run, and reusable semantic type metadata done; ongoing for full language |
...
| **1.4** | **Runtime Execution Engine:** Implement the core execution loop that traverses the AST, manages memory, resolves types, and executes lullaby instructions. | All previous steps | Critical | AST runtime with loops, range-for, arrays, and indexing done; native backend pending |
...
| **1.5.3** | **Offline Documentation Content Pass:** Bring the offline docs up to parity with the current language implementation: `.lby`, indentation-only blocks, functions, returns, `let`, assignment, control flow, memory builtins, CLI `check`/`run`, examples, and diagnostics. | 1.5.1, 1.5.2, Epic 1 | High | Initial content done; generated docs now cover fixture-backed examples plus package quick start, CLI reference, package layout, diagnostics, and current limitations; keep synchronized with Epic 1 |
| **1.5.4** | **Executable Example Sync:** Ensure examples shown in the offline docs are backed by fixtures or tests when they claim to work, and clearly mark future/planned syntax that is not yet accepted by the compiler. | 1.5.3, Epic 5 | High | Docs examples are fixture-backed and verified by `offline_docs/verify_offline_docs.py`; keep synchronized with Epic 1 |
...
| Story | Description | Dependencies | Estimated Effort | Status |
| :--- | :--- | :--- | :--- | :--- |
| **2.1** | **I/O System Integration:** Implement the I/O layer based on `lullaby_input_output.md`. Focus on file system access (reading/writing) for standard library operations. | `lullaby_input_output.md` | High | Text file builtins done: `read_file`, `write_file`, `append_file`, `file_exists`; standard stream builtins done: `print`, `println`, `warn` (stderr), `flush`; binary/mmap planned |
| **2.2** | **Error Handling Integration:** Integrate the error handling system (`lullaby_error_handling.md`) into the runtime to ensure all compilation and runtime errors are gracefully reported in a structured format. | `lullaby_error_handling.md` | Medium | Structured runtime/resource error formatting done; language-level try/catch planned |
| **2.3** | **System Call Abstraction:** Define the interface for executing low-level OS commands (e.g., system calls) that lullaby code can invoke safely. | N/A (New Design) | High | Direct program-plus-argv abstraction done: `sys_status`, `sys_output`; no shell invocation |
...
| **4.2** | **Installation & Setup:** Finalize the installation process, ensuring minimal user interaction and clear setup instructions are provided upon first launch. | 3.3, 4.1 | High | Portable archive setup instructions done in package README; optional user PATH integration helpers done for Windows and Unix portable archives; full installer pending |
...
| **5.3** | **Integration Test Suite:** Develop end-to-end integration tests that verify the entire pipeline: `Source Code` -> `AST` -> `Runtime Execution`. This ensures the compiler and runtime work together correctly. | All previous steps | Critical | In Progress; CLI backend smoke tests, optimizer CLI tests, and fixture-driven AST/IR/bytecode parity tests cover the current execution subset, including optimized backend variants |
| **5.4** | **Regression Test Protocol:** Establish a protocol for running the full suite (Unit + Integration) before any major feature addition or refactoring is committed to the codebase. | All previous steps | Medium | Release verification script done in `scripts/verify_release.ps1`; CI integration pending |
...
| Story | Description | Dependencies | Estimated Effort | Status |
| :--- | :--- | :--- | :--- | :--- |
| **6.1** | **Unified Diagnostic Model:** Define a shared diagnostic structure with code, phase/category, severity, source path, span/range, function/context, primary message, explanation, root cause, suggested fix, and related notes. | Current lexer/parser/semantic/runtime diagnostic structs | High | Shared `lullaby_diagnostics` crate done |
| **6.2** | **Verbose CLI Diagnostics:** Add `--verbose` diagnostics for `check` and `run` with source excerpts, caret markers, root-cause explanation, and fix guidance. | 6.1 | High | `--verbose` output done |
| **6.3** | **Machine-Readable Diagnostics:** Add deterministic JSON output for `check` and `run`, suitable for editors, CI, and LLM agents. | 6.1 | Medium | `--format json` and `--diagnostic-format json` output done |
| **6.4** | **Semantic Source Spans:** Extend semantic diagnostics so type, name, control-flow, and builtin argument errors point to the exact offending AST node instead of only the containing function. | 6.1, `lullaby_parser` spans | High | Semantic spans done for primary error paths |
| **6.5** | **Runtime Tracebacks:** Track source context and lightweight call stacks through user-defined functions and failing builtins/expressions. | 6.1, `lullaby_runtime` | High | Runtime source spans and call stack tracebacks done |
| **6.6** | **Diagnostic Registry:** Document every stable `L####` code with meaning, likely cause, example, and suggested fix; keep Markdown and offline browser docs synchronized. | 6.1 | Medium | Registry done in `diagnostic_registry.md` and offline docs |
| **6.7** | **Diagnostic Snapshot Tests:** Add representative CLI tests for lexer, parser, semantic, optimizer, bytecode/artifact, runtime, and resource diagnostics in concise, verbose, and JSON modes. | 6.2, 6.3 | High | CLI diagnostics coverage done, including `L0211` planned-syntax rejection and `L0329` executable entry-point validation |
| **6.8** | **IR Diagnostics:** Report IR lowering failures through the shared diagnostic model with phase, code, span, source path, and JSON/verbose rendering support. | 3.1, 6.1 | Medium | `L0501` IR lowering diagnostic done |
| **6.9** | **Bytecode Artifact Diagnostics:** Report malformed or unsupported compiled artifacts through the shared diagnostic model with phase, code, path, and JSON/verbose rendering support. | 3.1, 6.1 | Medium | `L0601` bytecode artifact diagnostic done |
