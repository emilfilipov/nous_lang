...
fn fuzz_alloc_cast_launder_native_always_skips() {
    ...
    //
    // MEASURED layer split over the 200 programs, asserted below rather than assumed:
    // 147 x `L0303` (the `let` wall), 41 x `L0301` (the cross-function shape, whose
    // refusal lands on the RETURN type because the identity cast yields `ptr_i64`), and
    // 33 x `L0339` (the native gate). Both the frontend and native populations are
    // required non-empty — "no exe" alone cannot distinguish a caught exploit from a
    // fuzzer that stopped reaching codegen, which is precisely how this test decayed
    // once before.
    //
    ...
    // Which LAYER refused each program. Counted, not just totalled, so the test can
    // prove it still reaches both — see the degradation guard after the loop.
    let mut frontend_refusals = 0u32;
    let mut native_skips = 0u32;
    ...
    for i in 0..PROGRAMS {
        ...
        // "No exe" is necessary but NOT sufficient, and alone it is a trap: a generator
        // shape with a typo ALSO produces no exe, so a broken generator is
        // indistinguishable from a caught exploit and this fuzzer stays green having
        // tested nothing. Pin the REASON — the model wall (`L0303`), the native skip
        // (`L0339`), or the interpreters' refusal of box arithmetic (`L0406`). A shape
        // that is merely malformed reaches none of them.
        // A parse/name error means the shape never expressed the exploit at all.
        for bogus in ["L0201", "L0211", "L0302"] {
            assert!(
                !listing.contains(bogus),
                "GENERATOR BROKEN on #{i} (seed {seed:#x}): {bogus} is a malformed-program \
                 error, so this shape tested nothing while still producing no exe — \
                 indistinguishable from a caught exploit:\n{source}\n{listing}"
            );
        }
        // The model rule lands on more than one wall, and which one is shape-dependent.
        // Measured over these 200 programs: 147 x `L0303` (the `let` wall), 41 x `L0301`
        // (the cross-function shape — `ptr_cast(p)` is now an identity yielding `ptr_i64`,
        // which mismatches the helper's declared `-> ptr<U>`, so the refusal lands on the
        // RETURN type instead), and 33 x `L0339` (the native gate). `L0406` is listed for
        // the interpreters' box-arithmetic refusal. `L0307` also appears (37 x) as
        // CASCADE noise from a `to_i64(i64)` wart in shape (1)'s pre-existing body; it
        // never appears alone, so it is not accepted as a reason on its own.
        assert!(
            ["L0301", "L0303", "L0313", "L0339", "L0406"]
                .iter()
                .any(|code| listing.contains(code)),
            "GENERATOR BROKEN on #{i} (seed {seed:#x}): no exe, but for none of the \
             reasons this fuzzer exists to check — expected a model wall (`L0301` return \
             type / `L0303` let / `L0313` parameter), a native skip (`L0339`), or \
             box-arithmetic refusal (`L0406`):\n{source}\n{listing}"
        );
        if listing.contains("L0303") {
            frontend_refusals += 1;
        }
        if listing.contains("skipped") || listing.contains("L0339") {
            native_skips += 1;
        }
    }

    // THE degradation guard, and the reason this fuzzer is written this way.
    //
    // Both populations must be non-empty. `gen_alloc_cast_launder_program` has already
    // silently decayed into a frontend-only test once: when `ptr_cast` was fixed, the
    // frontend began refusing shapes (1)-(4) before codegen, so the native gate went
    // unexercised — green throughout, teeth gone, while the doc still claimed it
    // guarded the gate. Counting only "no exe" cannot see that happen, because a
    // frontend refusal and a native skip are indistinguishable at the filesystem.
    //
    // So assert the SPLIT, not just the outcome: shapes (5)/(6) are model-preserving
    // and must still reach codegen for the native gate to refuse, and (1)-(4)/(7)-(11)
    // must still land on the frontend wall. If a future tightening makes every shape
    // frontend-rejected, `native_skips` hits zero and this FAILS loudly rather than
    // passing as a frontend-only test wearing a native-gate name.
    assert!(
        frontend_refusals > 0,
        "no shape hit the `L0303` model wall across {PROGRAMS} programs — the frontend \
         half of this fuzzer is no longer exercised"
    );
    assert!(
        native_skips > 0,
        "NO shape reached the NATIVE GATE across {PROGRAMS} programs: every program was \
         refused earlier, so this fuzzer has silently degraded into a frontend-only test \
         while still claiming to guard `refuse_legacy_box_pointer`. This exact decay \
         happened once before. Restore a model-preserving shape that reaches codegen."
    );
    eprintln!(
        "alloc launder fuzz: {PROGRAMS} programs — {frontend_refusals} frontend `L0303` \
         refusals, {native_skips} native-gate skips (both populations required non-empty)"
    );
}
